...
function normalizeMafiosopediaMediaSourceRootCode(
...
function assertPathInsideGameDataPatchesRoot(args: {
	...
	if (
		...
		throw new Error(
			"Resolved Mafiosopedia media path escaped GAME_DATA_PATCHES_ROOT.",
		);
	}
}
...
export function assertSafeMafiosopediaPatchCode(value: string): string {
	...
	if (!PATCH_CODE_PATTERN.test(normalized)) {
		throw new Error(
			"Mafiosopedia patch code is not safe for filesystem resolution.",
		);
	}
...
export function resolveMafiosopediaMediaAbsolutePath(args: {
	...
	const sourceRootCode = normalizeMafiosopediaMediaSourceRootCode(
		args.sourceRootCode,
	);
	...
	const rootFolder =
		sourceRootCode === "manual"
			? "Manual"
			: assertSafeMafiosopediaPatchCode(args.lastSeenPatchCode);
