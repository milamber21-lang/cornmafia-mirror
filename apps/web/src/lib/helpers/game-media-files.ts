//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/lib/helpers/game-media-files.ts                                                       ////
//// Language: TS                                                                                             ////
//// Safe filesystem and URL helpers for generated game-data media files.                                      ////
...
export function assertSafeGamePatchCode(value: string): string {
	const normalized = value.trim();
	if (!PATCH_CODE_PATTERN.test(normalized)) {
		throw new Error("Game patch code is not safe for filesystem resolution.");
	}

	return normalized;
}

export function assertSafeGameMediaId(value: string): string {
	const normalized = value.trim();
	if (!MEDIA_ID_PATTERN.test(normalized)) {
		throw new Error("Game media ID is not safe for lookup.");
	}

	return normalized;
}

export function buildGameMediaFileUrl(mediaId: string): string {
	const safeMediaId = assertSafeGameMediaId(mediaId);
	return `/api/game/media/${encodeURIComponent(safeMediaId)}`;
}

export function resolveGameMediaAbsolutePath(args: {
	lastSeenPatchCode: string;
	mediaRelPath: string;
}): string {
	const root = getGameDataPatchesRoot();
	const patchCode = assertSafeGamePatchCode(args.lastSeenPatchCode);
	...
	if (
		...
		throw new Error("Resolved game media path escaped GAME_DATA_PATCHES_ROOT.");
	}
