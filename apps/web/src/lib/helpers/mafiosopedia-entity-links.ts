...
export type MafiosopediaInfoRouteFamily =
	| "browse"
	| "sections"
	| "classes"
	| "categories"
...
export function normalizeMafiosopediaEntitySlug(
	entitySlug: string | null,
): string | null {
	const normalizedSlug = normalizePathSegment(entitySlug);
	if (
		!normalizedSlug ||
		!MAFIOSOPEDIA_ENTITY_SLUG_PATTERN.test(normalizedSlug)
	) {
		return null;
	}
...
export function buildMafiosopediaEntityHref(args: {
	...
	return entitySlug
		? `${MAFIOSOPEDIA_INFO_BASE_PATH}/entity/${entitySlug}`
		: null;
}

export function buildMafiosopediaMediaHref(
