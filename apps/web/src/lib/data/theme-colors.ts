...
export async function findThemeColorAdminById(themeColorId: number): Promise<ThemeColorAdminItem | null> {
  const result = await query<ThemeColorAdminDbRow>(
    `
      SELECT
        theme_color_id,
...
export async function listEnabledThemeColorOptions(): Promise<ThemeColorOption[]> {
  const result = await query<ThemeColorAdminDbRow>(
    `
      SELECT
        theme_color_id,
