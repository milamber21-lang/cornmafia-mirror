...
function mapDirectoryRow(row: RiseopediaClassificationDirectoryRow): RiseopediaHubDirectoryCardDoc {
	return {
...
function mapOptionRow(row: RiseopediaClassificationOptionRow): RiseopediaClassificationFilterOption {
	return {
...
export async function listRiseopediaSectionDirectoryCards(): Promise<RiseopediaHubDirectoryCardDoc[]> {
	const result = await query<RiseopediaClassificationDirectoryRow>(
		`SELECT 'section'::text AS node_type_code,
		 ...
		 FROM web_view.riseopedia_section_directory_rows section_row
