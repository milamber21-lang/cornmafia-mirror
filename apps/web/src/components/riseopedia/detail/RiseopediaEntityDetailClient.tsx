//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/components/riseopedia/detail/RiseopediaEntityDetailClient.tsx                                ////
//// Language: TSX                                                                                            ////
...
} from "@/components/riseopedia/detail/RiseopediaDetailLayout";
import RiseopediaEntityBodyContent from "@/components/riseopedia/detail/RiseopediaEntityBodyContent";
import RiseopediaEntityBodyRenderer from "@/components/riseopedia/detail/RiseopediaEntityBodyRenderer";
import RiseopediaEntityFooterBlocks from "@/components/riseopedia/detail/RiseopediaEntityFooterBlocks";
import RiseopediaEntityRecipeTree from "@/components/riseopedia/detail/RiseopediaEntityRecipeTree";
import RiseopediaEntityReleaseStatus, {
	type RiseopediaEntityReleaseStatusValue,
	type RiseopediaReleaseOverrideReasonOptionValue,
} from "@/components/riseopedia/detail/RiseopediaEntityReleaseStatus";
import RiseopediaMediaFrame from "@/components/riseopedia/ui/RiseopediaMediaFrame";
import { RiseopediaVariantLinkProvider } from "@/components/riseopedia/context/RiseopediaVariantLinkContext";
...
import RiseopediaOverviewTable, {
	renderableRiseopediaOverviewRows,
	type RiseopediaOverviewRow,
} from "@/components/riseopedia/detail/RiseopediaOverviewTable";
import type {
...
export type RiseopediaEntityDetailClientProps = {
	...
	wikiName?: string;
	releaseStatus?: RiseopediaEntityReleaseStatusValue | null;
	canManageReleaseStatus?: boolean;
	releaseOverrideReasons?: RiseopediaReleaseOverrideReasonOptionValue[];
	releaseFilters?: MafiosopediaReleaseFilterCode[];
	initialEntityVariantKey?: string | null;
};

type RiseopediaDetailHeroMediaRef = {
	mediaId: string;
	url: string;
	width: number | null;
	height: number | null;
...
const FALLBACK_HEADER_VARIANT_GROUPS = [
	"rarity",
	"tier",
	"color",
	"edition",
...
function selectedInitialVariant(args: {
	variants: RiseopediaEntityVariant[];
	initialEntityVariantKey: string | null | undefined;
}): RiseopediaEntityVariant | null {
	return (
		args.variants.find(
			(variant) => variant.variantKey === args.initialEntityVariantKey,
		) ??
		args.variants[0] ??
		null
	);
...
function variantRankFromValue(
	value: RiseopediaEntityVariantValue | undefined,
): number {
	if (!value) {
		return 100000;
	}
...
function fallbackVariantSelectorGroups(
	...
	for (const groupCode of FALLBACK_HEADER_VARIANT_GROUPS) {
		const matchingValue = variantValues.find(
			(row) => row.variantGroupCode === groupCode,
		);
		if (matchingValue) {
...
function selectedRepeatedElementsForSlot(args: {
	...
	return orderedDetailElements(
		Array.from(selectedByKey.values()).flatMap((group) =>
			uniqueDetailElements(group.rows),
		),
	);
}

function orderedDetailElements(
	rows: RiseopediaDetailElement[],
): RiseopediaDetailElement[] {
...
function orderedMediaCandidates(
...
function mediaCandidatesForVariant(args: {
	...
	return exact.length > 0 ? exact : shared;
}
...
function selectedMediaForVariant(args: {
	...
	return orderedMediaCandidates(mediaCandidatesForVariant(args))[0] ?? null;
}
...
function fallbackMediaForVariant(args: {
	media: RiseopediaEntityMediaRef[];
	selectedVariantId: string | null;
	primaryMediaFileId: string | null;
}): RiseopediaEntityMediaRef | null {
	if (!args.primaryMediaFileId) {
		return null;
	}

	const candidates = mediaCandidatesForVariant(args).filter(
		(row) => row.mediaFileId !== args.primaryMediaFileId,
	);
...
function heroMediaForFrame(
	row: RiseopediaEntityMediaRef | null,
): RiseopediaDetailHeroMediaRef | null {
	if (!row) {
		return null;
	}

	const heroUrl = row.heroUrl?.trim();
	if (!heroUrl) {
		return {
	...
	return {
		mediaId: row.heroMediaFileId ?? row.mediaFileId,
		url: heroUrl,
		width: row.heroWidth,
		height: row.heroHeight,
		mimeType: row.heroMimeType,
	};
}
	...
	return Array.from(groups.entries())
		.map(([key, groupRows]) => {
			...
			return {
				...
				value: overviewValue(orderedRows, wikiCode, releaseFilters, variantKeyFor),
			};
		})
		.sort((left, right) => {
...
function overviewValue(
	...
	if (rows.length === 1) {
		...
		return linkedValue(rows[0], wikiCode, releaseFilters, variantKeyFor);
	}
	...
	return (
		<ul className="riseopedia-overview-table__value-list">
			{rows.map((row) => (
				<li
					className="riseopedia-overview-table__value-item"
					key={detailElementKey(row)}
				>
...
function pushBreadcrumbItem(
	...
	if (
		previous &&
		normalizedBreadcrumbLabel(previous.label) ===
			normalizedBreadcrumbLabel(item.label)
	) {
		return;
	}
...
function classificationBreadcrumb(args: {
	...
	const withReleaseFilter = (href: string): string =>
		args.wikiCode === "mafiosopedia"
			? appendMafiosopediaReleaseSearchParam({
					href,
					filters: args.releaseFilters ?? mafiosopediaReleaseFiltersForView("all"),
				})
			: href;
	const items: RiseopediaBreadcrumbItem[] = [
		{
			label: args.wikiName,
			href: withReleaseFilter(
				buildRiseopediaInfoPath({
					family: "browse",
	...
	if (doc.entityClassName) {
		pushBreadcrumbItem(items, {
			...
			href:
				doc.entityTypeCode === "asset" && doc.entityClassCode
					? withReleaseFilter(
							buildRiseopediaInfoPath({
								family: "classes",
								slug: doc.entityClassCode,
								wikiCode: args.wikiCode,
							}),
						)
					: undefined,
			...
			href: doc.entitySubcategorySlug
				? withReleaseFilter(
						buildRiseopediaInfoPath({
							...
							slug: doc.entitySubcategorySlug,
							wikiCode: args.wikiCode,
						}),
					)
				: undefined,
...
export default function RiseopediaEntityDetailClient({
	...
	wikiName = "Riseopedia",
	releaseStatus = null,
	canManageReleaseStatus = false,
	releaseOverrideReasons = [],
	releaseFilters,
	initialEntityVariantKey = null,
}: RiseopediaEntityDetailClientProps): JSX.Element {
	const router = useRouter();
	const variantKeyByEntityVariantId = useMemo(
		() =>
			new Map(
				detail.variantLinkKeys.map((row) => [row.entityVariantId, row.variantKey]),
			),
		[detail.variantLinkKeys],
	);
	const variantKeyFor = useCallback(
		(entityVariantId: string | null | undefined): string | null =>
			entityVariantId
				? (variantKeyByEntityVariantId.get(entityVariantId) ?? null)
				: null,
	...
	const initialVariant = useMemo(
		() =>
			selectedInitialVariant({
				...
				initialEntityVariantKey,
			}),
		[initialEntityVariantKey, orderedVariants],
	);
	const [selectedVariantId, setSelectedVariantId] = useState<string | null>(
		initialVariant?.entityVariantId ?? null,
	);
	useEffect(() => {
		setSelectedVariantId(initialVariant?.entityVariantId ?? null);
	}, [detail.doc.entityId, initialVariant?.entityVariantId]);
	...
	const handleSelectVariant = useCallback(
		(variantId: string) => {
			const selected = orderedVariants.find(
				(variant) => variant.entityVariantId === variantId,
			);
			if (!selected) {
				return;
			}

			setSelectedVariantId(selected.entityVariantId);

			const href = buildRiseopediaEntityHref({
				entityTypeCode: detail.doc.entityTypeCode,
				entitySlug: detail.doc.entitySlug,
				targetEntityVariantKey: selected.variantKey,
				wikiCode,
				releaseFilters,
			});
		...
		[
			detail.doc.entitySlug,
			detail.doc.entityTypeCode,
			orderedVariants,
			releaseFilters,
			router,
			wikiCode,
	...
	const headerFallbackMedia = fallbackMediaForVariant({
		media: detail.media,
		selectedVariantId: effectiveVariantId,
		primaryMediaFileId: headerMedia?.mediaFileId ?? null,
	});
	const heroHeaderMedia = heroMediaForFrame(headerMedia);
	const heroHeaderFallbackMedia = heroMediaForFrame(headerFallbackMedia);
	...
	const resolvedOverviewRows = renderableRiseopediaOverviewRows(
		overviewRows(selectedOverviewRows, wikiCode, releaseFilters, variantKeyFor),
	);
	const configuredBodyRendererProps = {
		blocks: detail.bodyBlocks,
		selectedEntityVariantId: effectiveVariantId,
		currentEntity: detail.doc,
		currentEntityIconMediaFileId: detail.doc.primaryIconMediaFileId,
		variants: orderedVariants,
		variantValues: detail.variantValues,
		variantSelectors: detail.variantSelectors,
		media: detail.media,
		profileElements: bodyRows,
		recipeRequirements: detail.recipeRequirements,
		recipeOutputs: detail.recipeOutputs,
		locationTreeRows: detail.locationTreeRows,
		locationPoiRows: detail.locationPoiRows,
		perkTreeRows: detail.perkTreeRows,
		effectModifierRows: detail.effectModifierRows,
		experienceProgressionRows: detail.experienceProgressionRows,
		experienceLevelRows: detail.experienceLevelRows,
		experienceLevelUnlockRows: detail.experienceLevelUnlockRows,
		needEffectRows: detail.needEffectRows,
		poiLocationTreeRows: detail.poiLocationTreeRows,
		poiResourceYieldRows: detail.poiResourceYieldRows,
		poiTransportStopRows: detail.poiTransportStopRows,
		poiRelatedQuestRows: detail.poiRelatedQuestRows,
		poiPublicBenchLinkRows: detail.poiPublicBenchLinkRows,
		poiContainerLootRows: detail.poiContainerLootRows,
		poiSummaryFactRows: detail.poiSummaryFactRows,
		poiVendorStockRows: detail.poiVendorStockRows,
		questObjectiveRows: detail.questObjectiveRows,
		questRequirementRows: detail.questRequirementRows,
		questRewardRows: detail.questRewardRows,
		questFlowRows: detail.questFlowRows,
		mediaByFileId: mediaLookup,
		wikiCode,
		releaseFilters,
	};
	const configuredBody =
		detail.bodyBlocks.length > 0 ? (
			<RiseopediaEntityBodyRenderer
				{...configuredBodyRendererProps}
			...
			<>
				{detail.doc.entityTypeCode === "recipe" ? (
					<RiseopediaEntityRecipeTree
						...
						recipeStageElements={selectedOverviewRows}
						mediaByFileId={mediaLookup}
						wikiCode={wikiCode}
	...
	const hasConfiguredAside = detail.bodyBlocks.some(
		(block) => block.visible && block.displaySlotCode === "detail_aside",
	);
	const configuredAside = hasConfiguredAside ? (
		<RiseopediaEntityBodyRenderer
			{...configuredBodyRendererProps}
			placementCode="detail_aside"
		/>
	) : null;

	return (
		<RiseopediaEntityPreviewProvider
			currentEntitySlug={detail.doc.entitySlug}
			releaseFilters={releaseFilters}
			wikiCode={wikiCode}
		>
			<RiseopediaVariantLinkProvider variantLinkKeys={detail.variantLinkKeys}>
				<RiseopediaDetailLayout
					breadcrumb={classificationBreadcrumb({
						detail,
						wikiCode,
						wikiName,
						releaseFilters,
					...
					media={
						<RiseopediaMediaFrame
							media={heroHeaderMedia}
							fallbackMedia={heroHeaderFallbackMedia}
					...
					controls={
						releaseStatus ? (
							<div className="riseopedia-detail-hero__controls">
								<div className="riseopedia-detail-hero__status-row">
									<RiseopediaEntityReleaseStatus
										canManage={canManageReleaseStatus}
										reasons={releaseOverrideReasons}
										status={releaseStatus}
									/>
								</div>
								<div className="riseopedia-detail-hero__variant-row">
					...
					overview={
						resolvedOverviewRows.length > 0 ? (
							<RiseopediaOverviewTable
								rows={resolvedOverviewRows}
								rarityCode={selectedTone}
							/>
						) : null
					}
					body={configuredBody}
					aside={configuredAside}
					bottom={
