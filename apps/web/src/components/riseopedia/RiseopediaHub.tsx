...
function RiseopediaHubCardGroup({
	...
	return (
		<section className="riseopedia-classification-section public-collection-panel">
			<div className="riseopedia-classification-section__header">
				<div>
					<h2 className="riseopedia-classification-section__title">{title}</h2>
					<p className="riseopedia-classification-section__description">{description}</p>
				</div>
				{viewAllHref && (hasHiddenCards || visibleCards.length > 0) ? (
					<ButtonLink href={viewAllHref} variant="neutral">
						{viewAllLabel}
					</ButtonLink>
				) : null}
			</div>
...
export default function RiseopediaHub({
	...
	const hasActiveFilters = Boolean(section || entityClassCode || categorySlug || subcategorySlug);
	const hasActiveSearch = searchValue.length > 0;
	const hasActiveBrowser = hasActiveSearch || hasActiveFilters;
			...
			<div className="card public-collection-page riseopedia-page">
				...
				{hasActiveBrowser && entityResult ? (
					<RiseopediaEntityBrowser
						...
						search={search}
						basePath={basePath}
						title={searchValue ? `Search results: ${searchValue}` : "Filtered entries"}
