...
//// Language: TSX                                                                                               ////
//// Table-owned Riseopedia display profile admin list and row actions.                              ////
...
export interface DisplayProfileTableProps {
	initialRows: RiseopediaAdminRows;
	meta: RiseopediaAdminMeta;
}

export default function RiseopediaDisplayProfilesTable({ initialRows, meta }: DisplayProfileTableProps): JSX.Element {
	return (
		<DisplayProfilesTableBody
			...
			deleteConfirmMessage={(row) => `Delete display profile ${idText(row.display_profile_name)}?`}
			emptyText="No display profiles found."
			defaultSortKey="sort_order"
			filters={[entityTypeFilter(meta), activeFilter()]}
			rowActions={[
				{ label: "Bindings", href: (row) => `/admin/riseopedia/display-profiles/${idText(row.display_profile_id)}/bindings`, variant: "neutral" },
				{ label: "Elements", href: (row) => `/admin/riseopedia/display-profiles/${idText(row.display_profile_id)}/properties`, variant: "neutral" },
				{ label: "Selectors", href: (row) => `/admin/riseopedia/display-profiles/${idText(row.display_profile_id)}/variant-selectors`, variant: "neutral" },
			]}
			columns={[
...
export { RiseopediaDisplayProfilesTable };
...
interface DisplayProfilesTableBodyProps {
...
function DisplayProfilesTableBody({
	initialRows,
	apiPath,
	idKey,
	columns,
	...
	renderPanel,
}: DisplayProfilesTableBodyProps): JSX.Element {
