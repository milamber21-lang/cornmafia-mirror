//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/components/admin/riseopedia/RiseopediaReleaseDecisionsTable.tsx                    ////
//// Language: TSX                                                                                               ////
...
import RiseopediaAccessControlNav from "./RiseopediaAccessControlNav";
import {
	buildReleaseDecisionDetailHref,
	buildReleaseDecisionOverrideHref,
	createReleaseOverrideFromEvidence,
	hasManualReleaseOverride,
	idText,
	releaseDecisionClassificationFilters,
...
export interface RiseopediaReleaseDecisionsTableProps {
	initialRows: RiseopediaAdminRows;
	meta: RiseopediaAdminMeta;
	initialSearch?: string;
	initialFilterState?: RiseopediaAdminFilterState;
}

function rowIsLiveCandidate(row: { [key: string]: unknown }): boolean {
	const rawValue = idText(readRowValue(row, "effective_is_live_candidate"));
	return rawValue === "true" || rawValue === "Yes";
}

function isReleaseDecisionRow(value: unknown): value is TableRow {
	return typeof value === "object" && value !== null && !Array.isArray(value);
}

function readReleaseDecisionRowsPayload(payload: unknown): TableRow[] {
	if (typeof payload !== "object" || payload === null || !("rows" in payload)) {
		throw new Error("Release decisions response was missing rows.");
	}

	const rows = (payload as { rows?: unknown }).rows;
	if (!Array.isArray(rows)) {
		throw new Error("Release decisions response rows were invalid.");
	}

	return rows.filter(isReleaseDecisionRow);
}

async function fetchRiseopediaReleaseDecisionRows(): Promise<TableRow[]> {
	const response = await fetch("/api/admin/riseopedia/release-decisions", {
	...
	return readReleaseDecisionRowsPayload(payload);
}

export default function RiseopediaReleaseDecisionsTable({
	initialRows,
	meta,
	initialSearch,
	initialFilterState,
}: RiseopediaReleaseDecisionsTableProps): JSX.Element {
	return (
		<ReleaseDecisionsTableBody
			...
			columns={[
				{ rowKey: "entity_type_name", label: "Entity type" },
				...
				{ rowKey: "patch_label", label: "Patch" },
				{ rowKey: "entity_name", label: "Entity", strong: true },
				{ rowKey: "release_confidence_score", label: "Confidence" },
				{ rowKey: "effective_release_state_name", label: "Release state" },
			]}
		/>
	);
}

export { RiseopediaReleaseDecisionsTable };

interface ReleaseDecisionsTableBodyProps {
	initialRows: TableRow[];
	refreshRows?: () => Promise<TableRow[]>;
...
function ReleaseDecisionsTableBody({
	...
	initialSearch = "",
	initialFilterState,
}: ReleaseDecisionsTableBodyProps): JSX.Element {
