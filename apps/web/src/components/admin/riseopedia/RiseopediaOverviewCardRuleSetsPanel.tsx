//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/components/admin/riseopedia/RiseopediaOverviewCardRuleSetsPanel.tsx                               ////
//// Language: TSX                                                                                               ////
//// Dedicated Riseopedia overview card rule set panel.                                                    ////
...
export interface RiseopediaOverviewCardRuleSetsPanelProps {
	open: boolean;
...
export default function RiseopediaOverviewCardRuleSetsPanel({
	open,
	mode,
	row,
	meta,
	onClose,
	onSaved,
}: RiseopediaOverviewCardRuleSetsPanelProps): JSX.Element | null {
	...
	const fields = useMemo(
		() => buildRiseopediaOverviewCardRuleSetFields(meta),
		[meta],
	);
	...
	const handleSubmit = useCallback(
		async (values: { [key: string]: unknown }): Promise<void> => {
			...
			try {
				const response = await fetch("/api/admin/riseopedia/overview-card-rule-sets", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						op: "upsert",
						id: mode === "edit" && row ? row["overview_card_rule_set_id"] : null,
	...
	return (
		<PanelForm
			...
			title={mode === "create" ? "Create overview card rule set" : "Edit overview card rule set"}
			width="50%"
			showSave={true}
			mode={mode}
...
export { RiseopediaOverviewCardRuleSetsPanel };
