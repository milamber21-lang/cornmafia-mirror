//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/components/admin/riseopedia/RiseopediaProfileElementsTable.tsx                     ////
//// Language: TSX                                                                                               ////
//// Table-owned Riseopedia profile element admin list.                                              ////
...
import RiseopediaProfileElementsPanel from "./RiseopediaProfileElementsPanel";
import {
...
export default function RiseopediaProfileElementsTable({ initialRows, displayProfiles, meta, displayProfile, allBindings }: ProfileScopedProps): JSX.Element {
	const scopedProfileId = displayProfile ? idText(displayProfile.display_profile_id) : "";
	const fieldsBuilder: RiseopediaAdminFieldsBuilder = ({ row, rows }) => buildRiseopediaProfileElementFields({ displayProfiles, displayProfile, allBindings, meta, row, rows });

	return (
		<ProfileElementsTableBody
			initialRows={initialRows}
			apiPath={displayProfile ? `/api/admin/riseopedia/profile-properties?displayProfileId=${scopedProfileId}` : "/api/admin/riseopedia/profile-properties"}
			idKey="display_profile_element_id"
			searchPlaceholder="Search profile elements"
			createLabel="Add element"
			deleteConfirmTitle="Delete profile element"
			deleteConfirmMessage={(row) => `Delete element ${idText(row.element_label) || idText(row.source_label)} from this profile?`}
			emptyText="No profile elements found."
			...
			renderPanel={({ open, mode, row, rows, onClose, onSaved }) => (
				<RiseopediaProfileElementsPanel
					open={open}
					mode={mode}
					row={row}
...
export { RiseopediaProfileElementsTable };
...
interface ProfileElementsTableBodyProps {
...
function ProfileElementsTableBody({
	initialRows,
	apiPath,
	idKey,
	columns,
	...
	renderPanel,
}: ProfileElementsTableBodyProps): JSX.Element {
