//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// FILE: apps/web/src/components/admin/riseopedia/RiseopediaOverviewCardRuleElementsTable.tsx            ////
//// Language: TSX                                                                                               ////
//// Table-owned Riseopedia overview card rule element admin list.                                   ////
...
import RiseopediaOverviewCardRuleElementsPanel from "./RiseopediaOverviewCardRuleElementsPanel";
import {
...
export interface OverviewCardRuleElementsProps {
...
export default function RiseopediaOverviewCardRuleElementsTable({ initialRows, ruleSets, meta, ruleSet }: OverviewCardRuleElementsProps): JSX.Element {
	const scopedRuleSetId = ruleSet ? idText(ruleSet.overview_card_rule_set_id) : "";
	const fieldsBuilder: RiseopediaAdminFieldsBuilder = ({ row, rows }) => buildRiseopediaOverviewCardRuleElementFields({ ruleSets, ruleSet, meta, row, rows });

	return (
		<OverviewCardRuleElementsTableBody
			initialRows={initialRows}
			apiPath={ruleSet ? `/api/admin/riseopedia/overview-card-rule-elements?ruleSetId=${scopedRuleSetId}` : "/api/admin/riseopedia/overview-card-rule-elements"}
			idKey="overview_card_rule_element_id"
			searchPlaceholder="Search overview card elements"
			createLabel="Add element"
			deleteConfirmTitle="Delete overview card element"
			deleteConfirmMessage={(row) => `Delete element ${idText(row.display_slot_name) || idText(row.element_label) || idText(row.source_label)}?`}
			emptyText="No overview card elements found."
			defaultSortKey="sort_order"
			filters={[activeFilter()]}
			columns={[
				{ rowKey: "rule_set_label", label: "Rule set", strong: true },
				{ rowKey: "display_slot_name", label: "Slot" },
				{ rowKey: "source_type_name", label: "Source type" },
				{ rowKey: "element_label", label: "Element" },
				{ rowKey: "label_override", label: "Label override" },
				{ rowKey: "active_flag", label: "Status", kind: "status" },
			]}
			fields={[]}
			fieldsBuilder={fieldsBuilder}
			renderPanel={({ open, mode, row, rows, onClose, onSaved }) => (
				<RiseopediaOverviewCardRuleElementsPanel
					open={open}
					mode={mode}
					row={row}
						ruleSets={ruleSets}
...
export { RiseopediaOverviewCardRuleElementsTable };
...
interface OverviewCardRuleElementsTableBodyProps {
...
function OverviewCardRuleElementsTableBody({
	initialRows,
	apiPath,
	idKey,
	columns,
	...
	renderPanel,
}: OverviewCardRuleElementsTableBodyProps): JSX.Element {
