...
import L, { CRS, LatLngExpression, LatLng } from "leaflet";
// Leaflet CSS now loads from globals.css
...
function ClickDebugger({ maxZoom }: { maxZoom: number }) {
  useMapEvents({
    click(e) {
      const pt = CRS.Simple.latLngToPoint(e.latlng, maxZoom);
      // eslint-disable-next-line no-console
      console.log("click latlng:", e.latlng, "≈ pixels:", { x: pt.x, y: pt.y });
    },
  });
  return null;
}
  ...
  return (
    <div className="cm-map-wrap">
      <MapContainer
         ...
         center={centerLL}
         scrollWheelZoom
         attributionControl={false}   // ← hide bottom-right attribution control
       >
        <TileLayer
          url={tileUrl}
          tileSize={tileSize}
          ...
          eventHandlers={{
            tileerror: (ev) => {
              ...
              console.warn(
                "Tile error:",
                (ev as unknown as { tile?: HTMLImageElement }).tile?.src
              );
            },
          }}
