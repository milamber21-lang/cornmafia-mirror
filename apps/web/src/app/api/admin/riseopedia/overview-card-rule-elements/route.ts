...
export async function GET(request: NextRequest): Promise<Response> {
	...
	const scopedId = parsePositiveInt(request.nextUrl.searchParams.get("ruleSetId"));

	try {
		const rows = await listRiseopediaOverviewCardAdmin();
		const filteredRows = scopedId
			? rows.ruleElements.filter((row) => String(row.overview_card_rule_set_id ?? "") === String(scopedId))
			: rows.ruleElements;
...
export async function POST(request: NextRequest): Promise<Response> {
	...
	try {
		if (op === "upsert") {
			...
			const overviewCardRuleSetId = scopedId ?? getPositiveInt(data, "overviewCardRuleSetId");
			...
			if (!overviewCardRuleSetId || !displaySlotCode || !sourceTypeCode) {
				return jsonError("VALIDATION_REQUIRED", "Rule set, display slot, and source type are required.", 400);
			}
			...
			if (scopedId && postedRuleSetId && postedRuleSetId !== scopedId) {
				return jsonError("VALIDATION_REQUIRED", "Element rule set does not match selected rule set.", 400);
			}
		...
		if (op === "delete") {
			...
			await deleteRiseopediaOverviewCardRuleElementAdmin({ actorDiscordId: actorOrResponse, overviewCardRuleElementId });
