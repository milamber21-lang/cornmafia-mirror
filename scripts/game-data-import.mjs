...
const EXCLUDED_DIRECTORY_NAMES = new Set(["derived", "reports", "FullDump", "full_dumps", ".git", "node_modules"]);
const JSON_EXTENSION = ".json";
const MEDIA_EXTENSIONS = new Set([".png", ".jpg", ".jpeg", ".webp"]);
const DIRECT_MEDIA_PATH_PATTERN = /^(?:\/?)(?:Content|Plugins)\/.+\.(?:png|jpg|jpeg|webp)$/i;
const EMBEDDED_DIRECT_MEDIA_PATH_PATTERN = /(?:^|[\"'\s(,;])(?<path>\/?(?:Content|Plugins)\/[^\"'\s),;]+\.(?:png|jpg|jpeg|webp))/gi;
const UNREAL_QUOTED_REF_PATTERN = /[A-Za-z0-9_]+\'([^\']+)\'/g;
const UNREAL_BARE_REF_PATTERN = /(?<![A-Za-z0-9_])\/(?:Game|Engine|Corn[A-Za-z0-9_]+|[A-Za-z0-9_]+)\/[^\s\"',;)]+/g;
...
promote data into web_priv.game_* canonical tables.

The scanner only reads source roots by default. It ignores generated folders such as derived/ and reports/ so
...
function fileInfo(absolutePath, patchRootPath) {
	const relPath = normalizeRelPath(relative(patchRootPath, absolutePath).split(sep).join("/"));
	...
	return {
		...
		stem,
		ext: ext.replace(/^\./, ""),
		roleCode: ext === JSON_EXTENSION ? "json" : MEDIA_EXTENSIONS.has(ext) ? "media" : "other",
	...
	for (const table of tables) {
		...
		if (Array.isArray(table)) {
			for (let rowIndex = 0; rowIndex < table.length; rowIndex += 1) {
				...
				rows.push({
					sourceRowName: `${fallbackStem}_${rowIndex}`,
					sourceRowIndex: rowIndex,
...
function extractSourceIdentifierValue(payload) {
	...
	for (const key of ["DatatableIdentifier", "DataTableIdentifier", "Identifier", "Id", "ID", "Name", "DisplayName"] ) {
...
function extractReferences(rowPayload, sourceFileStem) {
	...
	function addReference(reference) {
		const key = [reference.refKindCode, reference.refRoleCode, reference.sourceRefValue, reference.targetMediaRelPath || "", reference.targetSourceRowName || ""].join("\u0000");
		if (seen.has(key)) {
			return;
		}
		seen.add(key);
	...
	function walk(value, path, parent) {
		if (typeof value === "string") {
			...
			for (const unrealMediaPath of mediaRelPathsFromUnrealReferences(value, path)) {
				addReference({
					refKindCode: "media",
					refRoleCode: mediaRoleFromPath(path),
					sourceRefValue: value,
...
function mediaRelPathFromUnrealReference(value) {
	...
	if (root === "Game") {
		return normalizeRelPath(join("Content", ...restParts).split(sep).join("/"));
	}

	if (PLUGIN_ROOTS.has(root)) {
		return normalizeRelPath(join("Plugins", root, "Content", ...restParts).split(sep).join("/"));
	}
...
function assetReferenceFromString(value, path, parent) {
	...
	const targetSourceFileCode = role === "recipe_component" || role === "recipe_output" ? "dt_inventoryitems" : null;
...
function normalizeFileCode(stem) {
	return stem.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
...
async function main() {
	...
	const sourceWalk = walkSourceFiles(patchRootPath, options.sourceRootsRelPaths);
	const files = sourceWalk.files.map((absolutePath) => fileInfo(absolutePath, patchRootPath));
	...
	try {
		...
		if (sourceWalk.missingRoots.length > 0) {
			const messageId = await insertMessage(client, {
				...
				context: { sourceRootsRelPaths: options.sourceRootsRelPaths, missingRoots: sourceWalk.missingRoots },
			});
		...
		for (const mediaFile of mediaFiles) {
			const fileResult = await insertImportFile(client, importBatchId, patchId, mediaFile, "processed", 0, {});
		...
		for (const jsonFile of jsonFiles) {
			const importFile = await insertImportFile(client, importBatchId, patchId, jsonFile, "pending", 0, {});
			let parsed;
			try {
			...
			await client.query(
				...
				[
					importFile.importFileId,
					parsedRowsForFile,
					JSON.stringify({ sourceFileCode, topLevelShape: Array.isArray(parsed) ? "array" : typeof parsed }),
				],
			);
		}
		...
		for (const reference of pendingReferences) {
			const resolution = resolveReference(reference, mediaByRelPath, mediaByLowerRelPath, rowsByFileCodeAndName, rowsByName);
			const refResult = await client.query(
				...
				[
					...
					resolution.message,
					JSON.stringify({ ...reference.metadata, sourceRelPath: reference.sourceRelPath, sourceRowName: reference.sourceRowName }),
				],
			);
			...
			if (resolution.statusCode === "missing" || resolution.statusCode === "ambiguous") {
				warningCount += 1;
				await insertMessage(client, {
					importBatchId,
					patchId,
					...
					messageText: resolution.message || `Reference ${reference.sourceRefValue} is ${resolution.statusCode}.`,
...
async function insertImportFile(client, importBatchId, patchId, file, parseStatusCode, rowCount, metadata) {
	const result = await client.query(
...
function resolveReference(reference, mediaByRelPath, mediaByLowerRelPath, rowsByFileCodeAndName, rowsByName) {
	...
	if (reference.refKindCode === "asset") {
		...
		if (reference.targetSourceFileCode) {
			const row = rowsByFileCodeAndName.get(`${reference.targetSourceFileCode}\u0000${targetName}`);
			if (row) {
				return {
					statusCode: "resolved",
